...
import com.siliconlabs.bluetoothmesh.App.Fragments.Provisioning.ordinary.Provisioning_nlc
...
constructor() : Fragment(R.layout.fragment_scanner) {
    companion object {
        ...
        fun newRemoteInstance(remoteProvisioner: Node, subnet: Subnet) =
            @Suppress("DEPRECATION")
            Scan_nlc().withMeshNavArg(remoteProvisioner.toNavArg(subnet))
    }
    ...
    private fun navigateToDeviceProvisioning(deviceToProvision: DeviceToProvision) {
        ...
        requireMainActivity().showFragment(Provisioning_nlc())
    }
}
