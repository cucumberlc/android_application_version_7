...
@Deprecated(
    ...
    replaceWith = ReplaceWith("SubnetFragment.newInstance(subnet)")
)
constructor() : Fragment(R.layout.fragment_subnet), SubnetView, MenuProvider {
    companion object {
        fun newInstance(subnet: Subnet) =
            @Suppress("DEPRECATION")
            SubnetFragment().withMeshNavArg(subnet.toNavArg())
    }
    ...
    private inner class SubnetPageAdapter : FragmentStateAdapter(this) {
        ...
        override fun createFragment(position: Int): Fragment {
            return when (position) {
                0 -> DeviceListFragment.newInstance(subnetPresenter.subnet, false)
                else -> AppKeysListFragment.newInstance(subnetPresenter.subnet, false)
            }
        }
    }
}
